...
1. Treat the current next step as attribution-driven investigation, not another
   cleanup cadence experiment. The fresh GC attribution sample measured
   `node_groups_remove` at 185.030s with `LargeShelfFactory` contributing
   139.219s and 5,661 removed node groups. The repeated top prefixes were
   `nodegroup_tagged_cube`, `nodegroup_division_board`, and
   `nodegroup_screw_head`.
2. Do not continue expanding `INFINIGEN_GC_NODE_GROUP_INTERVAL` experiments.
   ...
   creates large burst removes.
3. Inspect the factory paths and node tree generation for the factories that
   dominate node group removal, starting with `LargeShelfFactory`, then
   `SimpleBookcaseFactory`, `SimpleDeskFactory`, and the kitchen appliance
   factories observed in the attribution sample.
4. If repeated node group prefixes are semantically equivalent for the same
...
5. If inspection shows the node group names are highly parameterized or not
   ...
   A/B comparison.
6. Keep the standalone geometry kernels out of the default indoor solver
...
7. Treat `AssetFactory.spawn_asset` / factory lifecycle as the current first
   ...
   0.082%.
8. Treat `bpy.data.node_groups` removal as the current first behavior-preserving
...
9. Use the opt-in `INFINIGEN_GC_NODE_GROUP_INTERVAL` experiment only behind the
...
10. Do not treat the interval=20 smoke as a validated speedup. Both runs timed
...
11. If trying GC scope adjustment, node-group-specific cleanup, less frequent
