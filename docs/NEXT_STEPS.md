...
3. Run a complete same seed/gin/task A/B for the batch remove path before
   considering any mainline use. The smoke is not a pass: both runs timed out,
   `compare_indoor_outputs.py` found no comparable JSON, and `batch_remove` may
   change Blender internal deletion order or data-block lifecycle.
4. If a complete A/B still cannot finish, run a longer single-scene profile
   with the same seed/gin/task and keep the candidate opt-in. Require comparable
   coarse JSON before accepting behavior preservation.
5. Keep `LargeShelfFactory` and repeated node group prefixes as the root-cause
...
6. Do not continue expanding `INFINIGEN_GC_NODE_GROUP_INTERVAL` experiments.
...
7. If batch remove passes A/B, run a longer profile and explicit output
   ...
   delayed cleanup.
8. Inspect the factory paths and node tree generation for the factories that
...
9. If repeated node group prefixes are semantically equivalent for the same
...
10. If inspection shows the node group names are highly parameterized or not
   ...
   A/B comparison.
11. Keep the standalone geometry kernels out of the default indoor solver
...
12. Treat `AssetFactory.spawn_asset` / factory lifecycle as the current first
   ...
   0.082%.
13. Treat `bpy.data.node_groups` removal as the current first behavior-preserving
...
14. Use the opt-in `INFINIGEN_GC_NODE_GROUP_INTERVAL` experiment only behind the
...
15. Do not treat the interval=20 smoke as a validated speedup. Both runs timed
...
16. If trying GC scope adjustment, node-group-specific cleanup, less frequent
